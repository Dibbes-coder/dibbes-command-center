...
const fields: Array<{ key: keyof VoiceProfile; label: string; rows?: number }> = [
  ...
  { key: "coreTone", label: "Core signal" , rows: 3 },
  { key: "avoid", label: "Signal breakers to avoid", rows: 3 },
  { key: "signaturePhrases", label: "Signature signal words", rows: 2 },
  { key: "preferredLength", label: "Preferred reply length", rows: 2 },
  { key: "personalStance", label: "Position on X", rows: 3 },
];

export default function SignalSettings({ profile, onChange }: SignalSettingsProps) {
  ...
  return (
    <details className="surface-subtle group rounded-[1.75rem] p-5 sm:p-6">
      <summary className="flex cursor-pointer list-none items-center justify-between gap-4">
        <div>
          <p className="label text-gold/70">Signal memory</p>
          <h2 className="mt-2 text-xl font-semibold text-ivory">Signal Settings</h2>
          <p className="mt-2 text-sm leading-6 text-stone-400">
            Saved locally on this device. Your personal reply edge, not a generic voice preset.
          </p>
        </div>
